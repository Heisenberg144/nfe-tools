...
export async function openViewer(context: vscode.ExtensionContext) {
  ...
  const detection = detectFiscalModel(content);
  ...
  const itens = detMatches.map(det => {
    const extractDet = (tag: string) => det.match(new RegExp(`<${tag}[^>]*>([^<]+)</${tag}>`))?.[1] || '';
    return {
...
<body>
  ...
  <div class="section">
    ...
    <table>
      ...
      <tbody>
        ${data.itens.length > 0 ? data.itens.map((item: any) => `
          <tr>
